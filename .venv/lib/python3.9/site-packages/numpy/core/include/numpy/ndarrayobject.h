...
#define PyArray_HasArrayInterfaceType(op, type, context, out)                  \
  ((((out) = PyArray_FromStructInterface(op)) != Py_NotImplemented) ||         \
   ...
   (((out) = PyArray_FromArrayAttr(op, type, context)) != Py_NotImplemented))

#define PyArray_HasArrayInterface(op, out)                                     \
  PyArray_HasArrayInterfaceType(op, NULL, NULL, out)

#define PyArray_IsZeroDim(op)                                                  \
  (PyArray_Check(op) && (PyArray_NDIM((PyArrayObject *)op) == 0))
...
#define PyArray_IsPythonNumber(obj)                                            \
  (PyFloat_Check(obj) || PyComplex_Check(obj) || PyLong_Check(obj) ||          \
   PyBool_Check(obj))
...
#define PyArray_GETCONTIGUOUS(m)                                               \
  (PyArray_ISCONTIGUOUS(m) ? Py_INCREF(m),                                     \
   (m)                     : (PyArrayObject *)(PyArray_Copy(m)))

#define PyArray_SAMESHAPE(a1, a2)                                              \
  ((PyArray_NDIM(a1) == PyArray_NDIM(a2)) &&                                   \
   PyArray_CompareLists(PyArray_DIMS(a1), PyArray_DIMS(a2), PyArray_NDIM(a1)))
...
#define PyArray_ContiguousFromAny(op, type, min_depth, max_depth)              \
  PyArray_FromAny(op, PyArray_DescrFromType(type), min_depth, max_depth,       \
...
#define PyArray_SimpleNewFromData(nd, dims, typenum, data)                     \
  PyArray_New(&PyArray_Type, nd, dims, typenum, NULL, data, 0,                 \
...
#define PyArray_GETPTR3(obj, i, j, k)                                          \
  ((void *)(PyArray_BYTES(obj) + (i)*PyArray_STRIDES(obj)[0] +                 \
            (j)*PyArray_STRIDES(obj)[1] + (k)*PyArray_STRIDES(obj)[2]))
...
static NPY_INLINE void PyArray_DiscardWritebackIfCopy(PyArrayObject *arr) {
...
#define PyArray_FromObject(op, type, min_depth, max_depth)                     \
  PyArray_FromAny(op, PyArray_DescrFromType(type), min_depth, max_depth,       \
                  NPY_ARRAY_BEHAVED | NPY_ARRAY_ENSUREARRAY, NULL)
...
#define PyArray_CopyFromObject(op, type, min_depth, max_depth)                 \
  PyArray_FromAny(                                                             \
      op, PyArray_DescrFromType(type), min_depth, max_depth,                   \
      NPY_ARRAY_ENSURECOPY | NPY_ARRAY_DEFAULT | NPY_ARRAY_ENSUREARRAY, NULL)
...
static NPY_INLINE int NPY_TITLE_KEY_check(PyObject *key, PyObject *value) {
  PyObject *title;
